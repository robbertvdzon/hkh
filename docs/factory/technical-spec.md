...
- Backendmodules zijn Spring Modulith-modules; cross-module toegang loopt via de
  ...
  `dossier` niet; `aisearch` en `dossier` noemen `docexport` allebei in hun
  `allowedDependencies`, zodat er één renderpad voor alle PDF-exports blijft.
...
- Platformspecifiek gedrag zit achter een kleine abstractie met conditionele
  ...
  deeldialoog). Het AI-antwoordscherm en het artikelscherm gebruiken die ene
  saver; er komt per export geen tweede deelmechanisme bij. Widgettests
  injecteren daar een fake voor en draaien zo zonder browser-API's of
  Android-plugins.
