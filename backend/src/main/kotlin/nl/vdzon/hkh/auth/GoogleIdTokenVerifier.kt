...
class NimbusGoogleIdTokenVerifier(
    ...
    override fun verify(idToken: String): GoogleIdentity {
        ...
        if (clientId !in claims.audience.orEmpty()) throw unauthorized("Google ID token has the wrong audience")
        if (claims.issuer !in ACCEPTED_ISSUERS) throw unauthorized("Google ID token has the wrong issuer")
