...
@RestController
class AgentAccessController(private val access: AgentAccessVerifier, private val sessions: SessionService) {
    ...
    fun login(@RequestHeader("X-AI-Access-Token", required = false) token: String?,
        ...
        return mapOf("token" to result.token, "email" to result.user.email, "roles" to if (result.user.isAdmin) listOf("ADMIN") else emptyList<String>())
    }
...
<script>document.getElementById('login').onsubmit=async(event)=>{event.preventDefault();const field=document.getElementById('token');const status=document.getElementById('status');try{const result=await fetch('/api/auth/agent-session',{method:'POST',headers:{'Content-Type':'application/json','X-AI-Access-Token':field.value},body:JSON.stringify({email:document.getElementById('email').value})});field.value='';if(!result.ok)throw new Error('Aanmelden geweigerd ('+result.status+').');const data=await result.json();localStorage.setItem('flutter.hkh_user_session_token', JSON.stringify(data.token));
localStorage.setItem('flutter.hkh_user_session_email', JSON.stringify(data.email));
localStorage.setItem('flutter.hkh_admin_session_token', JSON.stringify(data.token));
